class SCBipolarConv2d(nn.Conv2d):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # --- Step 06: phase bases ---
        k_idx = torch.arange(CK, device=device, dtype=torch.int64)             # [CK]
        nl = N * L
        if self.phase_style == "const":
            ...
            base_x = (self.phase_x_base + l_idx + k_idx.view(1, CK)) % T         # [NL, CK]
        ...
        # --- Step 08.. : per-bitstream accumulation ---
        prod_bxbw = torch.zeros((nl, self.out_channels), dtype=torch.float32, device=device)
        for t in range(T):
            idx_x = (base_x + t * stride_x) % T                       # [NL, CK]
            idx_w = (base_w + t * stride_w) % T                       # [Cout, CK]
            bit_x = (idx_x < s_x)                                     # [NL, CK]
            bit_w = (idx_w < s_w)                                     # [Cout, CK]
            eq_k = (bit_x.unsqueeze(1) == bit_w.unsqueeze(0))         # [NL, Cout, CK]
            contrib = ((eq_k.float() * 2.0 - 1.0).sum(dim=2)) / float(T)
            prod_bxbw += contrib

            if self._gd.enable:
                # selectively dump to control size
                payload = {"t": t}
                if self._dbg_dump_bits:
                    payload.update({
                        "idx_x": idx_x, "idx_w": idx_w,
                        "bit_x": bit_x, "bit_w": bit_w,
                    })
                if self._dbg_dump_eq:
                    payload.update({"eq_k_packed": self._packbits_last(eq_k)})
                self._gd.dump(f"bitstep_t{t:04d}", **payload)

        self._gd.dump("prod_bxbw", prod_bxbw=prod_bxbw)
        ...
        self._gd.dump("decode_coeffs", A_x=float(A_x), B_x=float(B_x), A_w=A_w, B_w=float(B_w), sum_bw=sum_bw, CK=int(CK))

        # --- Step 10: reconstruct y_cols ---
        y_cols = (A_x * A_w) * prod_bxbw + (B_x * A_w) * sum_bw        # [NL, Cout]
        
        # equal = 
        # y_cols = A_w * prod_bxbw

        self._gd.dump("y_cols", y_cols=y_cols)

        # --- Step 11: fold back to feature map ---
